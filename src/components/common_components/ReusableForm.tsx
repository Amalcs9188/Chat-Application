  ...
  return (
    <form
        ...
        {fields.map((field) => {
          return (
            <div key={field.name} className="mb-[1rem] relative w-full">
              {field.type !== "checkbox" && (
                <Label className="p-2" htmlFor={field.name}>
                  {field.label}
                </Label>
              )}
              {(() => {
                if (field.type === "select") {
                  return (
                    <Select
                      ...
                      <SelectTrigger className="w-full border border-sidebar">
                  ...
                  if (field.multiple) {
                    return (
                      <SearchableSelect
                        ...
                        options={
                          field.options as { label: string; value: number }[]
                        }
                        onChange={(values) =>
                          setValue(
                            field.name,
                    ...
                    return (
                      <SearchableSelect
                        disabled={view}
                        value={watch(field.name) as number | undefined}
                        options={
                          field.options as { label: string; value: number }[]
                        }
                        onChange={(v) =>
                ...
                if (field.type === "textarea") {
                  return (
                    <textarea
                      className="w-full min-w-[350px] border-2 border-sidebar bg-white"
                      readOnly={view}
                      disabled={view}
                      id={field.name}
                      placeholder={field.placeholder}
                    ...
                    <div
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked) => {
                          setValue(
                            field.name,
                            checked as PathValue<FormInput, typeof field.name>,
                            {
                              shouldValidate: true,
                              shouldDirty: true,
                            }
                ...
                return (
                  <Input
                    className={`${
                      view
                        ? " focus:cursor-not-allowed focus:outline-none focus:border-none focus:ring-none"
                        : ""
                    } w-full min-w-[350px] border-2 border-sidebar bg-white`}
                    type={field.type}
                    id={field.name}
                    readOnly={view}
                    disabled={view}
      ...
      <div className="w-full mt-4 flex justify-end items-center">
        {!view && (
          <Button className="Button_class" variant="outline" type="submit">
            {submitButtonText}
          </Button>
        )}
      </div>
